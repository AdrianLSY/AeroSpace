...
* Floating windows are protected from being buried (`keep-floating-on-top`, on by default).
Hovering a tiling window while a floating window is focused will not raise the tiling window over it, and switching to a workspace that has a floating window brings it to the foreground.
See xref:#auto-raise-keep-floating-on-top[below].
...
[auto-raise]
    ...
    keep-floating-on-top    = true           # default: true
----
...
|Keep floating windows (dialogs, pickers, fixed-size popups) from getting buried. See xref:#auto-raise-keep-floating-on-top[below] for the two behaviors and their limits.
|===

[#auto-raise-keep-floating-on-top]
=== Keep floating windows on top

`keep-floating-on-top` (default `true`) stops floating windows from being buried under tiling windows. It has two effects:

* **Hover doesn't bury a float.** While a floating window is focused, moving the cursor over a tiling window does not raise that tiling window over the float — the hover-raise is dropped, so the float stays focused and on top.
* **Switching a workspace surfaces its float.** Switching to a workspace that has a floating window focuses the most-recently-used one, bringing it to the foreground (plain xref:./commands.adoc#focus[`focus`] onto a specific window is unaffected).

[NOTE]
====
This is a *soft* behavior, deliberately not a true "always on top":

* It is not modal. A real click, or focusing another window via keyboard / command, still moves focus off the float — only mouse *hover* is suppressed. Hovering from one floating window onto another is honored.
* macOS does not let an app pin another app's window above the frontmost app without disabling SIP (System Integrity Protection) and injecting code into `Dock.app` (the approach yabai uses). AeroSpace never does this, so "on top" here means "kept focused", not a raised window level. A window from a background app that steals focus on its own (for example a newly launched app) can still come forward — this feature only governs mouse-hover raises and workspace switches.
====
